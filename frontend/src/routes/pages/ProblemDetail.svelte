...
<div class="page">
  ...
  {#if loading}
    <div class="empty-state"><div class="spinner"></div><p>Загрузка задачи…</p></div>
  {:else if problem}
    <header class="header">
      <div class="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <span class="text-xs text-3">{problem.time_limit}s · {problem.memory_limit} MB · {problem.solved_count} решили</span>
      </div>
      <h1>{problem.title}</h1>
    </header>
    ...
    <div class="layout">
      <div class="statement">
        ...
        {#if problem.input_format}<h3>Входные данные</h3><p>{@html renderMarkdown(problem.input_format)}</p>{/if}
        {#if problem.output_format}<h3>Выходные данные</h3><p>{@html renderMarkdown(problem.output_format)}</p>{/if}
      ...
      <aside class="aside">
        ...
        {#if sampleTests.length === 0}
          <p class="text-3 text-sm">Сэмплы не опубликованы.</p>
        {:else}
          {#each sampleTests as test, i}
            <div class="test-case">
              <div class="test-case__header">Тест {i + 1}</div>
              <div class="mb-2"><strong class="text-xs text-3">Вход</strong><pre>{test.input_data}</pre></div>
              <div><strong class="text-xs text-3">Выход</strong><pre>{test.expected_output}</pre></div>
            </div>
          {/each}
        {/if}
        <button class="btn btn--primary btn--block btn--lg mt-6" on:click={solve}>Решить задачу →</button>
      </aside>
    </div>
  {/if}
</div>
